...
Server Actions คือ **asynchronous functions** ที่ทำงานบนฝั่ง server มันสามารถถูกใช้ได้ทั้งใน Server และ Client Components เพื่อจัดการการทำงานของ form submissions และ data mutations ใน Next.js

> **🎥 Watch:** เรียนรู้เพิ่มเติมเกี่ยวกับการทำงานของ forms และ mutations ด้วย Server Actions → [YouTube (10 minutes)](https://youtu.be/dDpZfOQBMaU?si=cJZHlUu_jFhCzHUg).

## Convention

เราสามารถกำหนด Server Action ใน React ด้วยการระบุ [`"use server"`](https://react.dev/reference/react/use-server) เราสามารถวาง `"use server"` ก่อนฟังก์ชัน `async` เพื่อกำหนดให้เป็นฟังก์ชันทำงานแบบ Server Action หรือ วาง `"use server"` ไว้บนสุดของไฟล์ เพื่อกำหนดให้การ export ของไฟล์นั้นทำงานแบบ Server Actions.

### Server Components

เราสามารถใช้ Server Components แบบ inline function level หรือ module level ได้โดยการใส่คำว่า `"use server"` ไว้ที่บรรทัดแรกของฟังก์ชันนั้น ดูตามตัวอย่างข้างล่าง :
...
Client Components สามารถนำเข้า actions ที่ระบุในระดับโมดูลว่าเป็น `"use server"`

ในการเรียกใช้ Server Action ใน Client Component จะต้องสร้างไฟล์ใหม่และใส่คำว่า `"use server"` ที่บรรทัดแรกของไฟล์ การทำแบบนี้จะทำให้ฟังก์ชันทั้งหมดในไฟล์นั้นถูกระบุให้ทำงานแบบ Server Actions ซึ่งการทำแบบนี้จะทำให้เราสามารถเรียกใช้ฟังก์ชันเหล่านี้ได้ทั้งในส่วนของ Client และ Server Components:
...
- การทำงานของ Server actions สามารถถูกเรียกใช้ได้โดยใช้ attribue `action` ใน [`<form>` element](#forms):
...
- Server Actions จะทำงานร่วมกับ [caching and revalidation](/docs/app/building-your-application/caching) ของ Next.js เมื่อถูกเรียกใช้ให้ทำงานมันจะสามารถคืนค่า UI ที่อัพเดทแล้ว และ ข้อมูลใหม่ ในครั้งเดียวกัน
- เบื้องหลังการทำงานนี้ คือการใช้ `POST` method ทำงาน และวิธีนี้ใช้ได้กับการเรียกใช้แบบ HTTP method เท่านั้น
- arguments และค่า return ของ Server Actions จะเป็นไปตามลำดับการทำงานของ React ดูข้อมูลเพิ่มเติมได้ที่ [serializable arguments and values](https://react.dev/reference/react/use-server#serializable-parameters-and-return-values).
- Server Actions เป็นฟังก์ชัน หมายความว่า เราสามารถนำไปใช้ซ้ำได้ในส่วนต่างๆของแอปพลิเคชัน
- Server Actions จะได้รับการสืบทอดใน [runtime](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes) จาก page หรือ layout ที่ถูกนำไปใช้
